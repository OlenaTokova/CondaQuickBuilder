...
#make blank array of packages to install for conda
conda_packages = []
#make blank array of packages to install for pip
pip_packages = []
...
def search_package(package, package_sign, package_version, channels_list):
    ...
    for channel in channels_list:
        ...
        if package in result.stdout.decode():
            conda_packages[package].append(channel)
            conda_packages[package].append(package_sign)
            conda_packages[package].append(package_version)
            return channel
        
    # Search in PIP
    version = search_pypi(package, package_sign, package_version)
    if version:
        pip_packages[package].append(version)
        return version
    else:
        return None
...
if __name__ == "__main__":
    ...
    if requirements:
        ...
        channels_packages_versions = ""
        for package in conda_packages:
            # make text for conda install command
            channels_packages_versions += f"-c {conda_packages[package][0]} {package}{conda_packages[package][1]}{conda_packages[package][2]} "
        ...
        for package in pip_packages:
            # make text for pip install command
            pip_packages_versions += f"{package}=={pip_packages[package][0]} "
        
        # Install pip packages
        # using this version "python -m pip install pandas==1.3.5"
        subprocess.run(["python", "-m", "pip", "install", pip_packages_versions], check=True)
