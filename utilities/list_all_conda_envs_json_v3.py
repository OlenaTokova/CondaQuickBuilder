...
If a name is available, it uses the `conda list --prefix {env_name}` command to get the list of packages in the environment.
...
for env in envs[3:-1]:
    ...
    if env_name:
        packages = subprocess.check_output(f"conda list --prefix {env_name}", shell=True).decode()
    else:
